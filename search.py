    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            if display:
                ...
                print('Number of generated nodes:', len(explored) + len(frontier))

                sum = 0
                b = 1
                d = len(node.state.schedule)
                while (abs(len(explored) + len(frontier) - sum) > 0.1 and len(explored) + len(frontier) > sum):
                    sum = 0
                ...
                print('Effective branching factor:', round(b, 3))                
            return node
        explored.add(node.state)
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
